...
describe('LandWorks', function () {
    ...
    describe('Diamond cut MetaverseConsumableAdapterFacet', async () => {
        ...
        describe('Diamond cut ListWithAdminFacet', async () => {
            let listWithAdminFacet: Contract;
            let listWithAdminFacetAddress: string;

            beforeEach(async () => {
                const facet = await Deployer.deployContract('ListWithAdminFacet');
                listWithAdminFacetAddress = facet.address;
                ...
                await landWorks.diamondCut(diamondAddFacet, ethers.constants.AddressZero, "0x");
                listWithAdminFacet = await ethers.getContractAt('ListWithAdminFacet', landWorks.address);
            });

            it('should have 9 facets', async () => {
                const actualFacets = await landWorks.facetAddresses();
                expect(actualFacets.length).to.be.equal(9);
                expect(actualFacets).to.eql([cut.address, loupe.address, ownership.address, marketplace.address, fee.address, erc721.address, decentraland.address, metaverseConsumableAdapterFacet.address, listWithAdminFacetAddress]);
            });
            ...
            it('should have correct function selectors linked to facet', async function () {
                ...
                const actualListWithAdminFacetSelectors = Diamond.getSelectorsFor(listWithAdminFacet);
                expect(await landWorks.facetFunctionSelectors(listWithAdminFacetAddress)).to.deep.equal(actualListWithAdminFacetSelectors);
            });
            ...
            it('should associate selectors correctly to facets', async function () {
                ...
                for (const sel of Diamond.getSelectorsFor(listWithAdminFacet)) {
                    expect(await landWorks.facetAddress(sel)).to.be.equal(listWithAdminFacetAddress);
                }
            });
            ...
            it('should return correct response when facets() is called', async function () {
                ...
                expect(facets[8].facetAddress).to.equal(listWithAdminFacetAddress);
                expect(facets[8].functionSelectors).to.eql(Diamond.getSelectorsFor(listWithAdminFacet));
            });

            describe('listWithAdministrativeConsumer', async () => {
                const secondAssetId = 1;
                const secondTokenId = 2;
                beforeEach(async () => {
                    await mockERC721Registry.mint(owner.address, secondTokenId);
                    // and:
                    await landWorks.setAdministrativeConsumerFor(mockERC721Registry.address, administrativeConsumer.address);
                    // and:
                    await mockERC721Registry.approve(landWorks.address, secondTokenId);
                });

                it('should successfully list with administrative consumer', async () => {
                    // given:
                    await landWorks.setConsumableAdapter(mockERC721Registry.address, metaverseAdapter.address);

                    // when:
                    await listWithAdminFacet
                        .listWithAdministrativeConsumer(
                            metaverseID,
                            mockERC721Registry.address,
                            secondTokenId,
                            minPeriod,
                            ...
                            pricePerSecond);
                    // then:
                    expect(await mockERC721Registry.ownerOf(secondTokenId)).to.equal(landWorks.address);
                    expect(await landWorks.ownerOf(secondAssetId)).to.equal(owner.address);
                    // and:
                    const asset = await landWorks.assetAt(secondAssetId);
                    expect(asset.metaverseId).to.equal(metaverseID);
                    expect(asset.metaverseRegistry).to.equal(mockERC721Registry.address);
                    expect(asset.metaverseAssetId).to.equal(secondTokenId);
                    expect(asset.paymentToken).to.equal(ADDRESS_ONE);
                    expect(asset.minPeriod).to.equal(minPeriod);
                    expect(asset.maxPeriod).to.equal(maxPeriod);
                    expect(asset.maxFutureTime).to.equal(maxFutureTime);
                    ...
                    expect(await landWorks.tokenOfOwnerByIndex(owner.address, 1)).to.equal(secondAssetId);
                    expect(await landWorks.tokenByIndex(1)).to.equal(secondAssetId);
                    // and:
                    expect(await metaverseAdapter.consumers(secondTokenId)).to.equal(administrativeConsumer.address);
                });
                ...
                it('should emit events with args', async () => {
                    ...
                    // when:
                    await expect(listWithAdminFacet
                        .listWithAdministrativeConsumer(
                            metaverseID,
                            mockERC721Registry.address,
                            secondTokenId,
                            ...
                            pricePerSecond))
                        .to.emit(landWorks, 'ConsumerChanged')
                        .withArgs(ethers.constants.AddressZero, ethers.constants.AddressZero, secondAssetId)
                        .to.emit(landWorks, 'Transfer')
                        .withArgs(ethers.constants.AddressZero, owner.address, secondAssetId)
                        .to.emit(mockERC721Registry, 'Transfer')
                        .withArgs(owner.address, landWorks.address, secondTokenId)
                        .to.emit(landWorks, 'List')
                        .withArgs(secondAssetId, metaverseID, mockERC721Registry.address, secondTokenId, minPeriod, maxPeriod, maxFutureTime, ADDRESS_ONE, pricePerSecond)
                        .to.emit(landWorks, 'UpdateAdapterAdministrativeConsumer')
                        .withArgs(secondAssetId, metaverseAdapter.address, administrativeConsumer.address)
                        .to.emit(metaverseAdapter, 'ConsumerUpdated')
                        .withArgs(secondTokenId, administrativeConsumer.address);
                });

                it('should revert when metaverse registry consumable adapter does not implement setConsumer', async () => {
                    // given:
                    await landWorks.setConsumableAdapter(mockERC721Registry.address, mockERC721Registry.address);
                ...
                it('should revert when metaverse registry consumable adapter is not set', async () => {
                    // when:
                    await expect(
                        listWithAdminFacet
                            .listWithAdministrativeConsumer(
                ...
                it('should revert when caller is not owner of the to-be-listed asset', async () => {
                    const expectedRevertMessage = 'ERC721: transfer caller is not owner nor approved';
                    // and:
                    await mockERC721Registry.approve(ethers.constants.AddressZero, secondTokenId);
            ...
            describe('listDecentralandWithAdministrativeOperator', async () => {
                const secondAssetId = 1;
                let landId: BigNumber;
                ...
                beforeEach(async () => {
                    ...
                    await landRegistry.authorizeDeploy(owner.address);
                    await landRegistry.assignNewParcel(x, y, owner.address);
                    landId = await landRegistry.encodeTokenId(x, y);
                    // and:
                    await landRegistry.approve(landWorks.address, landId);
                    // and:
                    await landWorks.updateAdministrativeOperator(administrativeOperator.address);
                ...
                it('should successfully list decentraland with administrative operator', async () => {
                    // when:
                    await listWithAdminFacet
                        .listDecentralandWithAdministrativeOperator(
                            metaverseID,
                            landRegistry.address,
                            landId,
                            minPeriod,
                    ...
                    // then:
                    expect(await landRegistry.ownerOf(landId)).to.equal(landWorks.address);
                    expect(await landWorks.ownerOf(1)).to.equal(owner.address);
                    // and:
                    const asset = await landWorks.assetAt(secondAssetId);
                    expect(asset.metaverseId).to.equal(metaverseID);
                ...
                it('should emit events with args', async () => {
                    ...
                    await expect(listWithAdminFacet
                        ...
                        .withArgs(ethers.constants.AddressZero, owner.address, secondAssetId)
                        .to.emit(landWorks, 'List')
                        .withArgs(secondAssetId, metaverseID, landRegistry.address, landId, minPeriod, maxPeriod, maxFutureTime, ADDRESS_ONE, pricePerSecond)
                        .to.emit(landWorks, 'UpdateAdministrativeState')
                        .withArgs(secondAssetId, administrativeOperator.address)
                        .to.emit(landRegistry, 'UpdateOperator')
                        .withArgs(landId, administrativeOperator.address)
                });

                it('should revert when registry is does not implement setOperator', async () => {
                    // given:
                    const secondTokenId = 2;
                    await mockERC721Registry.mint(owner.address, secondTokenId);
                    // and:
                ...
                it('should revert when trying to list a non-existing metaverse token id', async () => {
                    const invalidTokenId = 1234;
                    const expectedRevertMessage = ''; // LAND Registry does not have a message
                    // when:
                    await expect(listWithAdminFacet
                        .listDecentralandWithAdministrativeOperator(
                            metaverseID,
                ...
                it('should revert when caller is not owner of the to-be-listed asset', async () => {
                    const expectedRevertMessage = ''; // LAND Registry does not have a message
                    // and:
                    await landRegistry.approve(ethers.constants.AddressZero, landId);
